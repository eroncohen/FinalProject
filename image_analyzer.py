...
class FaceCropper(object):
    ...
    def generate(self, frame):
        ...
        The prediction comes back as a fraction between 0-1. The closest to 1 is the the closet the model detected a face
        '''
        if frame is None:
        ...
        for (x, y, w, h) in faces:
            if w < 30 and h < 30:  # skip the small faces (probably false detections)
                continue
            face_img = self.crop_face(gray_image, x, y, w, h)
    ...
    def crop_face(self, gray_image, x, y, w, h):
        r = max(w, h) / 2
        center_x = x + w / 2
        center_y = y + h / 2
        nx = int(center_x - r)
